...
#   -------------------      Target-Table       -------------------  #
# Target-Table
TARGET_TABLE_NAME = "sales_mktg.cds_fleet_out_of_stock"
TARGET_TABLE_PATH = f"/sync/{ENV}_42124_edw_b/EDW/SALES_MKTG/RZ/CDS_FLEET_OUT_OF_STOCK/Processing"
...
# Drop duplicates and sum up Day_Sales_Cnt, while picking the first value for Mth_To_Dt_Sales_Cnt
# and include the curr_mth_start_dt column
df_MERGED = df_SQL.groupBy("Rpt_Dt").agg(
    ...
    F.first("curr_mth_start_dt", ignorenulls=True).alias("Curr_Mth_Start_Dt")
)
...
df_FINAL = (
    ...
    .withColumn('Rpt_Mth_Nbr', F.date_format(F.col('Curr_Mth_Start_Dt'), 'MM').cast('int'))
    .withColumn('Rpt_Mth_Abbr_Txt', F.date_format(F.col('Curr_Mth_Start_Dt'), 'MMM'))
    # PLUMBING COLUMNS
